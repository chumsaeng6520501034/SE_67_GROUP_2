                            ...
                            <div class="p-6 flex-1">
                                <form action="/corpDetailMyoffer" method="POST">
                                    ...
                                    <h2 class="text-2xl font-bold text-black-600 hover:text-blue-500">
                                        ...
                                        <button type="submit"> {{ ucwords($payments->name) }} </button>
                                    </h2>
                                </form>
                                <p class="text-gray-600 mt-1">{{ $payments->description }}</p>
                                @switch($payments->status)
                                    @case('ongoing')
                                        <p class="text-[#007BFF] text-sm mt-2 font-bold">{{ ucwords($payments->status) }}</p>
                                    @break

                                    @case('cancal')
                                        <p class="text-[#FF5733] text-sm mt-2 font-bold">{{ ucwords($payments->status) }}</p>
                                    @break

                                    @case('finish')
                                        <p class="text-[#28A745] text-sm mt-2 font-bold">{{ ucwords($payments->status) }}</p>
                                    @break

                                    @case('collect')
                                        <p class="text-[#FFC107] text-sm mt-2 font-bold">{{ ucwords($payments->status) }}</p>
                                    @break
                                @endswitch
                                <p class="text-gray-400 text-xs mt-1">Start Date: {{ $payments->start_tour_date }}</p>
                                <p class="text-gray-400 text-xs  mt-1">End Date: {{ $payments->end_tour_date }}</p>
                            </div>
                            <div class="p-6 bg-gray-100 w-1/4 text-right rounded-r-lg">
                                <p class="text-gray-800 text-md font-bold">Release Date</p>
                                <p class="text-gray-600 font-semibold">{{ $payments->Release_date }}</p>
